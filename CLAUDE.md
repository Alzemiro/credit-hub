...
- **Azure Key Vault**: fonte real dos segredos (`kafka-jaas`, `sr-auth`, `pg-password`), populado pelo TF a partir dos outputs do Confluent e de `var.postgres_admin_password`. Os Container Apps recebem esses secrets **inline** (`value` do KV), **não** via `key_vault_secret_id` + managed identity: o `secret.identity` (KV ref) dispara `IdentityDoesNotExist` no create atômico do ACA (a UAI é validada antes de ser associada ao app). Como os valores já vivem no tfstate, inline não expõe nada novo. Além disso, **nenhum** vínculo com a UAI fica no create do TF (nem `identity {}`, nem registry): associar a UAI no create do ACA dispara `IdentityDoesNotExist`. O **CD** faz `az containerapp identity assign` + `registry set` após o create (updates, confiáveis); o TF usa `ignore_changes = [identity, registry, image]`. Ver [DECISIONS.md](DECISIONS.md) item 7.
